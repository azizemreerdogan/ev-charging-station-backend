...
function assertAdminOrOperator(actor) {
...
export async function patchUser(actor, id, patch) {
    assertAdmin(actor);
...
function operatorWhere(actor) {
    return actor.role === "OPERATOR" ? { operatorId: actor.userId } : {};
}
export async function listStations(actor, filters) {
    ...
    const enriched = items.map((s) => ({
        ...
        offlineConnectors: s.connectors.filter((c) => c.status === "OFFLINE" || c.status === "MAINTENANCE").length,
    }));
...
async function loadStationForActor(actor, stationId) {
...
export async function patchStation(actor, id, patch) {
    assertAdminOrOperator(actor);
...
// Connectors
export async function listConnectorsForStation(actor, stationId) {
    assertAdminOrOperator(actor);
...
export async function createConnector(actor, stationId, body) {
    assertAdminOrOperator(actor);
    await loadStationForActor(actor, stationId);
    ...
    if (existing) {
        throw new ConflictError(`Connector number ${body.connectorNumber} already exists at this station`);
    }
...
async function loadConnectorForActor(actor, connectorId) {
...
export async function patchConnector(actor, id, patch) {
    assertAdminOrOperator(actor);
    ...
    if (patch.status &&
        ...
        throw new ConflictError(`Cannot change status while connector is ${connector.status}`);
    }
...
export async function deleteConnector(actor, id) {
    assertAdminOrOperator(actor);
    const connector = await loadConnectorForActor(actor, id);
    if (connector.status === "IN_USE" || connector.status === "RESERVED") {
        throw new ConflictError(`Cannot delete connector in status ${connector.status}`);
    }
...
// Sessions oversight
export async function listSessions(actor, filters) {
    assertAdminOrOperator(actor);
    const where = {
...
export async function overview(actor) {
    ...
    const sessionScope = actor.role === "OPERATOR"
        ? { connector: { station: { operatorId: actor.userId } } }
        : {};
    const stationScope = operatorWhere(actor);
    const reservationScope = actor.role === "OPERATOR"
        ? { connector: { station: { operatorId: actor.userId } } }
        : {};
    const invoiceScope = actor.role === "OPERATOR"
        ? {
            session: {
                connector: { station: { operatorId: actor.userId } },
            },
        ...
        : {};
    const paymentScope = actor.role === "OPERATOR"
        ? {
            session: {
                connector: { station: { operatorId: actor.userId } },
            },
        ...
        : {};
    const [activeSessions, todayReservations, todayInvoices, weekPayments, totalStations, totalConnectors,] = await Promise.all([
    ...
    const paymentsByStatus = Object.fromEntries(weekPayments.map((p) => [p.status, p._count]));
...
export async function stationAnalytics(actor, range) {
    assertAdminOrOperator(actor);
    const stationWhere = operatorWhere(actor);
    const stations = await prisma.chargingStation.findMany({
        where: stationWhere,
        select: { id: true, name: true, _count: { select: { connectors: true } } },
    });
    const sessionWhere = (stationId) => ({
        connector: { stationId },
        ...(range.from || range.to
            ? {
                startTime: {
...
export async function paymentAnalytics(actor, range) {
    assertAdminOrOperator(actor);
    const where = {
        ...(actor.role === "OPERATOR"
            ? {
...
export async function adminEmergencyStop(actor, sessionId) {
    assertAdminOrOperator(actor);
    const session = await prisma.chargingSession.findUnique({
        where: { id: sessionId },
    ...
    if (session.status !== "ACTIVE") {
        throw new ConflictError(`Session is already in status ${session.status}`);
    }
    // Delegate to the existing emergency-stop pipeline so finalize/billing/payment
    // logic stays in one place.
    const { emergencyStopByOperator } = await import("../sessions/sessions.service.js");
