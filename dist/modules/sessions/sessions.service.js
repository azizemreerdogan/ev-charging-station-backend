...
import { calculateCost, generateInvoiceNumber, } from "../billing/billing.service.js";
 ...
 */
function estimateEnergyKwh(startTime, endTime, powerKw) {
    const hours = (endTime.getTime() - startTime.getTime()) / 3_600_000;
    return Math.max(0, hours * powerKw * ENERGY_EFFICIENCY);
}
export async function startSession(userId, reservationId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (reservation.status !== "CONFIRMED") {
            throw new ConflictError(`Cannot start session for reservation in status ${reservation.status}`);
        }
...
export async function getSessionLive(userId, sessionId) {
    ...
    const energyKwh = isActive
        ? estimateEnergyKwh(session.startTime, referenceEnd, session.connector.powerKw)
        : session.energyKwh;
    const durationMinutes = (referenceEnd.getTime() - session.startTime.getTime()) / 60_000;
...
export async function stopSession(userId, sessionId, opts = {}) {
    const session = await prisma.chargingSession.findUnique({
        where: { id: sessionId },
        include: { connector: true },
    });
...
export async function emergencyStopByOperator(actorUserId, sessionId) {
    ...
    const isStationOperator = actor.role === "OPERATOR" &&
        session.connector.station.operatorId === actor.id;
    const isOwner = session.userId === actor.id;
    if (!isAdmin && !isStationOperator && !isOwner) {
        throw new ForbiddenError("Only the session owner, the station operator, or an admin may emergency-stop");
    }
    if (session.status !== "ACTIVE") {
        throw new ConflictError(`Session is already in status ${session.status}`);
    }
...
async function finalizeSession(sessionId, finalStatus, opts = {}) {
    return prisma.$transaction(async (tx) => {
        ...
        const energyKwh = estimateEnergyKwh(session.startTime, endTime, session.connector.powerKw);
        const durationMinutes = (endTime.getTime() - session.startTime.getTime()) / 60_000;
...
export async function listHistory(userId, filters = {}) {
