...
import { buildStaticMapUrl, getDistances, isGoogleMapsEnabled, } from "../../services/google-maps.js";
...
const createStationSchema = z
    ...
    (v.latitude !== undefined && v.longitude !== undefined), { message: "latitude and longitude must be provided together" });
...
function mapStatusFor(stationStatus, availableCount) {
...
export async function stationRoutes(app) {
    ...
    r.get("/", { schema: { querystring: discoveryQuerySchema, tags: ["stations"] } }, async (req) => {
        ...
        if (q.lat !== undefined && q.lng !== undefined) {
            distances = await getDistances({ lat: q.lat, lng: q.lng }, filtered.map((s) => ({
                lat: s.latitude,
                lng: s.longitude,
            })));
        }
        const enriched = filtered
            ...
            .filter((s) => (s.distanceKm === null || s.distanceKm <= q.radiusKm) &&
    ...
    r.get("/map", {
        ...
        const markers = stations
            ...
            if (q.lat === undefined || q.lng === undefined)
                return true;
            return (haversineKm(q.lat, q.lng, m.latitude, m.longitude) <=
                q.radiusKm);
        });
        ...
        if (!url) {
            return reply.code(503).send({
                ...
                message: "Static Map rendering requires GOOGLE_MAPS_API_KEY. Use the lat/lng/markers data with a client-side SDK instead.",
            });
        }
        return { url };
    });
    ...
    r.get("/:id/static-map", {
        ...
        if (!url) {
            return reply.code(503).send({
                code: "GOOGLE_MAPS_DISABLED",
                message: "Static Map rendering requires GOOGLE_MAPS_API_KEY",
                fallback: {
                    latitude: station.latitude,
                    longitude: station.longitude,
                    mapStatus: status,
