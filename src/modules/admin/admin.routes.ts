...
function actor(req: FastifyRequest): svc.Actor {
...
export async function adminRoutes(app: FastifyInstance) {
    ...
    r.get(
        "/users",
        { ...guard, schema: { querystring: userListQuerySchema, tags: ["admin"] } },
        async (req) => svc.listUsers(actor(req), req.query),
    );
    r.get(
        "/users/:id",
        { ...guard, schema: { params: idParamSchema, tags: ["admin"] } },
        async (req) => svc.getUser(actor(req), req.params.id),
    );
    r.patch(
        "/users/:id",
        {
    ...
    r.get(
        ...
        { ...guard, schema: { params: idParamSchema, tags: ["admin"] } },
        async (req) =>
            svc.listConnectorsForStation(actor(req), req.params.id),
    );
    r.post(
        "/stations/:id/connectors",
        {
        ...
        async (req, reply) => {
            const c = await svc.createConnector(
                actor(req),
                req.params.id,
                req.body,
            );
    ...
    r.patch(
        ...
        async (req) =>
            svc.patchConnector(actor(req), req.params.id, req.body),
    );
    r.delete(
        "/connectors/:id",
        { ...guard, schema: { params: idParamSchema, tags: ["admin"] } },
        async (req) => svc.deleteConnector(actor(req), req.params.id),
    );
    ...
    r.get(
        "/sessions/:id",
        { ...guard, schema: { params: idParamSchema, tags: ["admin"] } },
        async (req) => svc.getSession(actor(req), req.params.id),
    );
    r.post(
        "/sessions/:id/emergency-stop",
        { ...guard, schema: { params: idParamSchema, tags: ["admin"] } },
        async (req) => svc.adminEmergencyStop(actor(req), req.params.id),
    );

    // ---- Analytics ----
    r.get(
        "/analytics/overview",
        { ...guard, schema: { tags: ["admin"] } },
        async (req) => svc.overview(actor(req)),
    );
    r.get(
        "/analytics/stations",
        {
