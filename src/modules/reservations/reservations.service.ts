...
export async function createReservation(input: CreateReservationInput) {
    ...
    if (durationMin < MIN_DURATION_MINUTES) {
        throw new ValidationError(
            `Reservation must be at least ${MIN_DURATION_MINUTES} minutes`,
        );
    }
    if (durationMin > MAX_DURATION_MINUTES) {
        throw new ValidationError(
            `Reservation cannot exceed ${MAX_DURATION_MINUTES} minutes`,
        );
    }
    const leadHours =
    ...
    if (leadHours > MAX_LEAD_HOURS) {
        throw new ValidationError(
            `Reservations must start within ${MAX_LEAD_HOURS} hours`,
        );
    }
    ...
    if (!vehicle || vehicle.deletedAt) throw new NotFoundError("Vehicle");
    ...
    if (!vehicle.connectorTypes.includes(connector.type)) {
        throw new ValidationError(
            `Vehicle does not support connector type ${connector.type}`,
        );
    }
    ...
    if (activeCount >= MAX_ACTIVE_PER_USER) {
        throw new ConflictError(
            `Maximum ${MAX_ACTIVE_PER_USER} active reservations per user`,
        );
    }
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (overlap) {
            throw new ConflictError(
                "Connector already reserved for the requested window",
            );
        }
...
export async function cancelReservation(userId: string, reservationId: string) {
    ...
    const minutesUntilStart =
        (reservation.startTime.getTime() - Date.now()) / 60_000;
    if (minutesUntilStart < CANCEL_DEADLINE_MINUTES) {
        throw new ConflictError(
            `Cancellation must be at least ${CANCEL_DEADLINE_MINUTES} minutes before start`,
        );
    }
...
export function listUserReservations(userId: string) {
